...
emailInput.addEventListener('blur', () => {
  ...
  if (emailValido) {
    ...
    xhr.open('POST', 'database/verifica_email.php', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
      if (this.status == 200) {
        const   
 emailExiste = this.responseText === 'true';
        if (emailExiste) {
...
form.addEventListener('submit', (event) => {
  ...
  if (!erro) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'database/processa_login.php', true); 
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
      if (this.status == 200) {
        if (this.responseText === 'sucesso') {
          ...
          alert('Erro ao fazer login/registrar. ' + this.responseText);
        }
      }
    }
